...
function genSubAllocHex(res, base, digits, childNum) {
  ...
  res++;
  newDigits.push(Math.floor(childNum / 7) % 7);
  res++;
  newDigits.push(Math.floor(childNum / (7 * 7)) % 7);
  res++;
  newDigits.push(Math.floor(childNum / (7 * 7 * 7)) % 7);
  res++;
  newDigits.push(1);
  while (res < 15) {
    res++;
  ...
  // console.log('Jim childNum, digits', childNum, newDigits.join('_'));
...
// console.log(output)
writeFileSync('../vichex-search-experiment-1.csv', output);
